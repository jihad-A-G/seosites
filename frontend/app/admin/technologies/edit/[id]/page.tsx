...
export default function EditTechnologyPage() {
  ...
  useEffect(() => {
    if (technologyData?.data) {
      const tech: Technology = technologyData.data;
      setFormData({
        name: tech.name,
  ...
  const updateTechnologyMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await api.put(`/technologies/${technologyId}`, data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['technologies'] });
      queryClient.invalidateQueries({ queryKey: ['technology', technologyId] });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateTechnologyMutation.mutate(formData);
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 space-y-6">
        ...
        <div className="flex gap-4">
          ...
          >
            {updateTechnologyMutation.isPending ? 'Updating...' : 'Update Technology'}
          </button>
          <button
            type="button"
            onClick={() => router.push('/admin/technologies')}
